...
class TodoBox extends React.Component {
  ...
  handleChange(event) {
    ...
    request(baseUrl + '/update/' + date + '/' + completed + '/' + todo).then(reponse => {
      console.log('updated')
...
class App extends React.Component {
  ...
  loadList() {
    request(baseUrl + '/get/all').then(todos => this.setState({ todos: JSON.parse(todos) })).catch(e => { console.log(e) })
  }
  ...
  handleSubmit(event) {
    ...
    if (todo)
      request(baseUrl + '/save/' + todo).then(response => {
        console.log('saved')
  ...
  handleButton(event) {
    ...
    request(baseUrl + '/remove/' + deleteTodo).then(resp => {
      console.log('removed')
    }).catch(e => { console.log(e) })
    // loads the list again to update the UI
    this.loadList()
  ...
  render() {
    ...
    return <div>
      <h3 className="text-center">MangoHacks Todos</h3>
      ...
      {todos.map(({todo, completed, date}) => <div key={date} style={styles.center}>
        <TodoBox date={date} completed={completed} todo={todo} updateList={this.loadList}/>
      </div>)}
